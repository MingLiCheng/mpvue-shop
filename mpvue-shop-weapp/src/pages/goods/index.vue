  ...
  <div class="goods">
    ...
    <div class="goods-info">
      <div class="c">
        ...
        <p :class="{'is-so':isSo==1}">￥{{info.retail_price}}</p>
          <p v-if="isSo==1" style="color: #EC4700;">￥{{soInfo.special_offer_price}}</p>
    ...
    <div class="common-problem">
      ...
      <div class="sublist">
        <div @click="togoodsDetail(subitem.id)" v-for="(subitem,index) in productList" :key="index">
...
export default {
  ...
  mounted() {
    ...
    this.isSo = this.$root.$mp.query.isSo;

    this.openId = getStorageOpenid();
    this.goodsDetail(this.isSo);
  },
  //商品转发
  onShareAppMessage() {
    ...
    async bug() {
      if (toLogin()) {
        if (this.showpop) {
          ...
          if (data) {
            wx.navigateTo({
              url: `/pages/order/main?type=2&goodsId=${this.goodsId}&goodsNum=${this.number}&goodsPrice=${this.allPrise}`,
            });
          }
        } else {
          this.showpop = true;
