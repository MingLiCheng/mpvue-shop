...
async function detailAction(ctx) {
  ...
  const isSo = ctx.query.isSo;
  //商品信息
  const info = await mysql('nideshop_goods').where({
    'id': goodsId
  }).select();
  let soInfo;
  if(isSo == 1){ // 特价商品查询特价信息
    soInfo = await mysql('nideshop_special_offer_goods').select('special_offer_price', 'special_goods_num').where('goods_id', goodsId).first()
  }
  //商品相关图片
