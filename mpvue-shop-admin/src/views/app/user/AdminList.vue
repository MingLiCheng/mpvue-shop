  ...
  <section>
    <div class="table_container">
      ...
      >
        ...
        <el-table-column prop="nickName" label="昵称">
          <template slot-scope="scope">
            <span style="color:#4db3ff">
              <a href="javascript:;" style="color: #4db3ff">{{ scope.row.username }}</a>
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="avatarUrl" label="邮件">
          <template slot-scope="scope">
            {{scope.row.email}}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="id" label="ID" align="center" width="320"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getUserList() {
      this.$axios.get('/list/adminList').then(res => {
        console.log('res',res);
